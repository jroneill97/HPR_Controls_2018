...
%% Find the thrust at the current time
    ...
    for j = 1:7
      T(j) = motorCluster(j).thrust(index);
    end   
    totalThrust   = sum(T,2); % sum of all the thrusts from each motor
...
%% Solve the ODE    
    [tNew,tempStates] = ode45(@(tNew,statesIC) linearizedEquationsOfMotion(statesIC,...
                                                                 totalThrust,currentFwind_x,currentFwind_y),...
